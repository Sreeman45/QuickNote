import { AlignJustify, Home, Star } from "lucide-react";
...
export default function Sidebar() {
  ...
  const fetchdata = async () => {
    const user = await checkAuth("/data/mydata", {
      method: "GET",
      credentials: "include",
    });
    console.log(user);
  ...
  return (
    <>
      <div className=" flex flex-col flex-wrap w-[18%] h-screen justify-start items-center border-gray-500 border-x-2 border-opacity-30 ">
        <div className="h-24 flex flex-wrap justify-center text-xl items-center  ">
          <span className="h-10 w-8 bg-purple-700 border-2 border-purple-900 mr-3 rounded-3xl font-bold "></span>
          AI Notes
        </div>
        <div
          onClick={fetchdata}
          className="hover:text-purple-700 h-16 w-[100%] rounded-full flex flex-wrap justify-start items-center hover:bg-slate-300 bg-opacity-80 cursor-pointer font-bold text-gray-600"
        >
          <Home
        ...
        <div className="hover:text-purple-700 h-16 w-[100%] rounded-full flex flex-wrap min-w-[10%] justify-start items-center  hover:bg-slate-300 cursor-pointer font-bold text-gray-400">
          <Star className="mr-2 ml-10" />
          Favorates
        </div>
