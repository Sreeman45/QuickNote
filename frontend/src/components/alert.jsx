...
export default function Alert({ id, date, title, description }) {
  ...
  const deleteNote = (id) => {
    fetch(`data/delete/${id}`, { method: "DELETE" })
      .then((res) => res.json())
      .then((data) => {
        show.current?.close();
        return (data.message = "success" ? seti((prev) => prev + 1) : null);
      })
  ...
  return (
    <>
      <dialog ref={show}>
        <div className="border-[1.5px] p-3 py-4 max-h-[70vh] sm:max-md:max-h-[100vh] max-w-[70vw]   border-gray-500 no-scrollbar rounded overflow-y-scroll ">
          <div className="flex gap-3 justify-between  ">
            <h3 className="text-xl font-semibold font-serif overflow-y-scroll no-scrollbar ">
              {title}
            </h3>
            <div className="flex gap-2">
              <Minimize2
                className="size-5 cursor-pointer"
                onClick={() => show.current?.close()}
              />

              <Trash
                className="size-5 hover:-translate-y-1 ease-in cursor-pointer hover:text-red-600"
      ...
      <div className="border-[1.5px] p-3 h-32 w-72  border-gray-500 mt-4 ml-4 rounded hover:shadow-lg">
        <div className="flex gap-3 justify-between">
          ...
          <div className="flex gap-2">
            <Pencil
              className="size-5 cursor-pointer"
              onClick={() => updateNote(title, description)}
            />
            <Expand
              className="size-5 cursor-pointer"
              onClick={() => show.current?.showModal()}
            />
